...
> Оригинальный проект поддерживается командой внутри компании 1С-Битрикс.
> 
> См. [MODIFICATIONS.md](MODIFICATIONS.md) для списка внесенных изменений.
...
- сервис для бесплатных SSL-сертификатов от `LetsEncrypt`:
  - используем стабильный образ `goacme/lego:v4.28.1`
  - добавляем логику слоем сверху
  - собираем `bitrix24/lego:4.28.1-v1-alpine`
...
- `bitrix24/lego`:
```bash
cd env-docker/sources/bxlego4281/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/lego:4.28.1-v1-alpine --no-cache .
```
